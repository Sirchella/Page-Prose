...
def _send(*, to: str, subject: str, html: str) -> None:
    ...
    try:
        ...
        resend.Emails.send({
            "from": getattr(settings, 'FROM_EMAIL', 'noreply@pageandprose.com'),
            "to": [to],
            "subject": subject,
            "html": html,
        })
...
def send_order_confirmation(order) -> None:
    ...
    body = f"""
      ...
      <div style="background-color:#F5EFE7;border:2px solid #D4C4B0;border-radius:8px;padding:16px 24px;margin-bottom:28px;text-align:center;">
        <p style="margin:0;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;">Order Number</p>
        <p style="margin:6px 0 0;font-size:22px;color:#4A7C2C;font-family:Georgia,serif;font-weight:bold;">#{order.id}</p>
      </div>
      <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
        <thead>
          <tr>
            <th style="text-align:left;padding-bottom:8px;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #D4C4B0;">Book</th>
            <th style="text-align:center;padding-bottom:8px;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #D4C4B0;">Qty</th>
            <th style="text-align:right;padding-bottom:8px;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #D4C4B0;">Price</th>
          </tr>
        </thead>
      ...
      <div style="text-align:right;margin-bottom:28px;">
        ...
        <span style="font-size:20px;color:#4A7C2C;font-family:Georgia,serif;font-weight:bold;">{round(float(order.total_price)):,} XAF</span>
      </div>
      <div style="background-color:#F5EFE7;border-left:4px solid #4A7C2C;padding:16px 20px;border-radius:0 8px 8px 0;margin-bottom:28px;">
        <p style="margin:0 0 4px;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;">Delivery Address</p>
        <p style="margin:0;font-size:14px;color:#2C2416;font-family:Arial,sans-serif;line-height:1.6;">{order.shipping_address.replace(', ', '<br />')}</p>
      </div>
...
def send_status_update(order) -> None:
    ...
    status_config = {
        "shipped": {
            ...
            "message": "Your books have been packed and handed to our courier.",
            "badge_color": "#F59E0B", "badge_label": "Shipped",
        },
        "delivered": {
            "emoji": "✅", "headline": "Your order has been delivered!",
            "message": "We hope your new books bring you hours of joy.",
            "badge_color": "#4A7C2C", "badge_label": "Delivered",
        },
    }
    ...
    body = f"""
      ...
      <h2 style="margin:0 0 12px;font-size:22px;color:#2C2416;font-family:Georgia,serif;">{cfg['headline']}</h2>
      <p style="margin:0 0 28px;color:#6B5D4F;font-family:Arial,sans-serif;font-size:15px;line-height:1.6;">{cfg['message']}</p>
      <div style="background-color:#F5EFE7;border:2px solid #D4C4B0;border-radius:8px;padding:16px 24px;margin-bottom:28px;text-align:center;">
        <p style="margin:0;font-size:12px;color:#6B5D4F;font-family:Arial,sans-serif;text-transform:uppercase;letter-spacing:1px;">Order Number</p>
        <p style="margin:6px 0 0;font-size:22px;color:#4A7C2C;font-family:Georgia,serif;font-weight:bold;">#{order.id}</p>
      </div>
...
def send_test_email(to: str) -> None:
    ...
    """
    _send(to=to, subject="Test Email | Page & Prose", html=_base_layout("Test Email", body))
