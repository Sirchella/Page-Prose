...
import { createBook, type Book } from '../api';
...
export function ProductDrawer({ isOpen, onClose, onSave }: ProductDrawerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to add book. Check all fields and try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-2xl bg-[#1a1a1a] z-50 shadow-xl overflow-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          {error && (
            <div className="bg-red-900/20 border border-red-700/40 text-red-400 text-sm rounded-lg px-4 py-3">
              {error}
            </div>
          )}
