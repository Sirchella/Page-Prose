              ...
              {order.items.map((item) => (
                <div key={item.id} className="flex gap-4 pb-4 border-b border-[#262626] last:border-0 last:pb-0">
                  ...
                  <div className="text-right">
                    <p className="text-sm text-green-400">{Math.round(item.price * item.quantity).toLocaleString()} XAF</p>
                    <p className="text-xs text-[#a3a3a3] mt-1">{Math.round(item.price).toLocaleString()} XAF each</p>
                  </div>
                </div>
              ))}
            </div>
